...
function toggleSeen(state = initialState, action) {
  ...
  switch (action.type) {
    case "TOGGLE_SEEN":
      const seenFilmIndex = state.seenFilm.findIndex(
        item => item.id === action.value.id
      );
      if (seenFilmIndex !== -1) {
        // Le film est déjà dans les films déjà vus, on le supprime de la liste
        nextState = {
          ...state,
          seenFilm: state.seenFilm.filter(
            (item, index) => index !== seenFilmIndex
          )
        };
      } else {
